...
// GET /api/reservations/dispatch - Get active assignments for driver and volunteer dispatch
router.get('/dispatch', async (req, res) => {
  try {
    const query = {
      deliveryStatus: { $in: ['pending', 'claimed', 'pickup_scheduled', 'picked_up', 'in_transit'] }
    };
    ...
    const reservations = await Reservation.find(query)
      .populate({
        path: 'requestRef',
        populate: [
          { path: 'ingredientRef', populate: { path: 'donorRef', select: 'name email location contactPerson' } },
          { path: 'soupKitchenRef', select: 'name email location' }
        ]
      })
      .sort({ createdAt: -1 })
      .limit(50);
    ...
    const sanitized = reservations.map(r => {
      const obj = r.toObject();
      // Keep pickupCode hidden from general view unless needed for verification
      delete obj.pickupCode;
      return obj;
    });
    ...
    console.error('Fetch dispatch reservations error:', error);
    res.status(500).json({ message: 'Internal server error while fetching dispatch reservations.' });
  }
});

// PUT /api/reservations/:id/verify-pickup - Verify 6-digit pickup code
router.put('/:id/verify-pickup', authorizeRoles('donor', 'soup_kitchen', 'admin'), verifyLimiter, async (req, res) => {
  try {
    ...
    const result = await runWithTransaction(async (session) => {
      ...
      if (!request || !request.ingredientRef) {
        return { status: 404, message: 'Associated request/ingredient not found.' };
      }

      if (request.ingredientRef.donorRef.toString() !== req.user.id) {
        return { status: 403, message: 'Access denied. You do not own the ingredient for this reservation.' };
      }
      ...
      if (reservation.pickupConfirmedByDonor || !reservation.pickupCode || reservation.pickupCode === 'used') {
        return { status: 400, message: 'This pickup code has already been verified and cannot be reused.' };
      }

      // Maximum failed attempts check
      if (reservation.failedAttempts >= 3) {
        return { status: 400, message: 'Code has been locked due to too many failed attempts. Please regenerate the code.' };
      }

      // Code expiry check
      if (reservation.codeExpiresAt && new Date() > new Date(reservation.codeExpiresAt)) {
        return { status: 400, message: 'Pickup code has expired. Please regenerate the code.' };
      }
      ...
      if (!matches) {
        ...
        return { status: 400, message: 'Pickup code does not match. Do not release the ingredient.' };
      }
    ...
    const result = await runWithTransaction(async (session) => {
      ...
      if (req.user.role === 'donor') {
        ...
        if (request.soupKitchenRef.toString() !== req.user.id) {
          return { status: 403, message: 'Access denied. You do not own this reservation.' };
        }
      }
      ...
      if (deliveryStatus === 'pickup_scheduled' && current !== 'claimed') {
        return { status: 400, message: 'Cannot transition to pickup_scheduled unless current status is claimed.' };
      }

      if (deliveryStatus === 'completed') {
        if (current !== 'handed_over') {
          return { status: 400, message: 'Cannot transition to completed unless current status is handed_over.' };
        }
        if (!reservation.pickupConfirmedByDonor) {
          return { status: 400, message: 'Pickup code has not been verified by the donor. Status transition to completed is blocked.' };
        }
      }

      if (deliveryStatus === 'cancelled' && !['claimed', 'pickup_scheduled'].includes(current)) {
        return { status: 400, message: 'Cannot cancel reservation after it has been handed over or completed.' };
      }
      ...
      if (deliveryStatus === 'completed') {
        ...
        if (request.ingredientRef) {
          ...
          await request.ingredientRef.save(session ? { session } : {});
        }
        ...
        if (kitchen) {
          ...
          await kitchen.save(session ? { session } : {});
        }
        ...
        await donorNotification.save(session ? { session } : {});
...
router.post('/:id/regenerate-code', authorizeRoles('soup_kitchen', 'admin'), async (req, res) => {
  try {
    ...
    const result = await runWithTransaction(async (session) => {
      ...
      if (!request) {
        return { status: 404, message: 'Associated request not found.' };
      }
      ...
      if (reservation.pickupConfirmedByDonor) {
        return { status: 400, message: 'Cannot regenerate code for a reservation that has already been verified/picked up.' };
      }
